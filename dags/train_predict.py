...
# Use the existing Airflow connection you already created
...
def train(cur):
    """
    1) Build a simple training view from the raw table
    2) Create the Snowflake.ML Forecast object
    3) Show evaluation metrics
    """
    train_input_table      = "RAW_SCHEMA.STOCK_PRICE_AMAZON_META"
    train_view             = "ADHOC.MARKET_DATA_TRAIN_VIEW"
    forecast_function_name = "ANALYTICS_SCHEMA.PREDICT_STOCK_PRICE"
...
def predict(cur):
    ...
    final_table            = "ANALYTICS_SCHEMA.MARKET_DATA"
...
with DAG(
    ...
    catchup=False,
    schedule='30 3 * * *',      # run after your ETL
    tags=['ML', 'Forecast']
) as dag:
